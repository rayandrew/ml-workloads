...
class Preprocessor:
    ...
    def pad_to_min_shape(image, label):
        ...
        return np.pad(image, paddings, mode="edge"), np.pad(
            label, paddings, mode="edge"
        )
    ...
    def load_pair(self, case: str):
        ...
        image, label = (
            ...
            label.get_fdata().astype(np.uint8),
        )
    ...
    def resample3d(self, image, label, image_spacings):
        if image_spacings != self.target_spacing:
            ...
            image = interpolate(
                torch.from_numpy(np.expand_dims(image, 0)),
                size=new_shape,
                mode="trilinear",
                align_corners=True,
            )
            label = interpolate(
                torch.from_numpy(np.expand_dims(label, 0)),
                size=new_shape,
                mode="nearest",
            )
    ...
    def save(self, image, label, case: str):
        ...
        mean, std = (
            np.round(np.mean(image, (1, 2, 3)), 2),
            np.round(np.std(image, (1, 2, 3)), 2),
        )
        ...
        np.save(
            os.path.join(self.results_dir, f"{case}_x.npy"), image, allow_pickle=False
        )
        np.save(
            os.path.join(self.results_dir, f"{case}_y.npy"), label, allow_pickle=False
        )
