    ...
    if local_rank == 0:
        if not flags.benchmark:
            ...
            if flags.save_ckpt_path:
                callbacks.append(
                    CheckpointCallback(
                        flags.save_ckpt_path, metric="mean_dice", seed=flags.seed
                    )
                )
        else:
            callbacks.append(
                PerformanceCallback(
                    logger,
                    flags.batch_size * world_size * flags.ga_steps,
                    flags.warmup_steps,
                    mode="train",
                )
            )
...
class EvaluationCallback(BaseCallback):
    ...
    def on_epoch_end(self, epoch, metrics, *args, **kwargs):
        ...
        if (
            self._best_metrics[self._prefix + self._main_metric]
            < metrics[self._main_metric]
        ):
            for key in metrics.keys():
                self._best_metrics[self._prefix + key] = float(metrics[key])

        if (
            metrics[self._main_metric] >= self._threshold
            and self._first_epoch_above_threshold == 0
        ):
        ...
        self._logger.log(
            step=(metrics["epoch"]), data={**metrics, **self._best_metrics}
        )
        self._logger.flush()
...
class CheckpointCallback(BaseCallback):
    ...
    def on_epoch_end(self, epoch, metrics, model, optimizer, *args, **kwargs):
        ...
        self._last_state = {
            "last_model_state_dict": current_state_dict,
        ...
        if metrics[self._main_metric] > self._best_metric:
            ...
            self._best_state = {
                "best_model_state_dict": copy.deepcopy(current_state_dict),
    ...
    def on_fit_end(self, *args, **kwargs):
        torch.save(
            {**self._last_state, **self._best_state, "seed": self._seed}, self._path
        )
