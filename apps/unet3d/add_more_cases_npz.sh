...
DATASET_DIR="/p/lustre5/sinurat1/dataset/ml-workloads/unet3d-npz"
PARALLEL_JOBS=64

echo "Counting source cases..."
mapfile -t source_files < <(find "$DATASET_DIR" -maxdepth 1 -name "case_*_x.npz")
...
create_link() {
    ...
    SRC_X="$2"
    DATASET_DIR="$3"
    SRC_Y="${SRC_X/_x.npz/_y.npz}"

    DEST_X="$DATASET_DIR/case_$(printf '%05d' $TARGET_CASE)_x.npz"
    DEST_Y="$DATASET_DIR/case_$(printf '%05d' $TARGET_CASE)_y.npz"
